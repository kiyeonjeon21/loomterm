...
## 2026-07 검증: public preview packaging과 replay

v0.3.0을 세 native target의 attested prerelease와 Homebrew tap으로 배포했다.
`loom init`은 기존 Codex TOML과 Claude JSON을 보존하면서 project MCP 설정을
추가한다. 실제 Codex/Claude Code fixture에서 총 14개 MCP execution을 네 recorded
session에 연결했고, 11개 성공과 의도한 3개 non-zero가 누락이나 truncation 없이
남았다. Codex session 하나는 self-contained replay와 41초 MP4로 GitHub Pages에
공개했다.

release와 onboarding 가설은 구현·검증됐지만, 이번 agent 작업은 각각 20분에
미달하므로 다음 제품 계층을 결정할 근거로 과장하지 않는다. 확인된 후속 마찰은
Codex의 explicit project trust, Claude의 first-run MCP approval 및 integer schema
format 경고, headless tool filtering이다. 다음 실제 장시간 작업에서는 이 마찰과
input-required/human handoff 발생 여부를 함께 측정한다.

## 2026-07 자동 probe의 잠정 가설: packaging/onboarding

기본 로컬 DB가 비어 있는 상태에서 약 2분 31초의 자동 probe를 수행했다. CLI
10건과 실제 Codex MCP 5건, 총 15건이 discovery, metadata, fmt/clippy, 전체
테스트, release build, git 확인, shell pipeline, non-zero, cancel, cursor
reconnect를 포괄했다. 이는 30~45분 실제 작업 세션을 대체하지 않는다.
...
현재 선두 가설은 **versioned binary 배포, 설치 경로, workspace/MCP bootstrap을
묶은 packaging/onboarding**이다. 다만 PTY/input-required/human handoff와
sandbox의 우선순위를 배제하는 최종 결정은 아니다. runtime-contract와 storage
onboarding 문제를 먼저 닫고, 실제 작업 세션에서 다시 판정한다. 상세 측정값과
판정 기준은 `DOGFOOD.md`에 보존한다.
